...
def refresh_watchlist_from_nasdaq(db: Session):
    syms = fetch_tech_microcaps(limit_cap=int(settings.MARKETCAP_LIMIT))
    if not syms:
        return 0
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if settings.WATCHLIST_REFRESH_ON_START and settings.AUTO_FILTER_TECH:
            try:
                refresh_watchlist_from_nasdaq(_db)
            except Exception:
                pass
    finally:
        _db.close()
...
@app.post("/admin/refresh_watchlist")
def admin_refresh_watchlist(x_admin_token: str = Header(default="")):
    if x_admin_token != settings.ADMIN_TOKEN:
        raise HTTPException(status_code=401, detail="Unauthorized")
    db = SessionLocal()
    try:
        n = refresh_watchlist_from_nasdaq(db)
        return {"updated": n}
    finally:
        db.close()
